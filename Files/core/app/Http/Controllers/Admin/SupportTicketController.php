...
use App\Models\SupportCreditMessage;
use Auth;
...
class SupportTicketController extends Controller
{
    ...
    public function creditreply($id)
    {
        $ticket = SupportCredit::with('user')->where('id', $id)->firstOrFail();
        $page_title = 'Acquista Crediti Request';
        $messages = SupportCreditMessage::with('ticket')->where('supportticket_id', $ticket->id)->latest()->get();
        return view('admin.support.reply', compact('ticket', 'messages', 'page_title'));
    }
    ...
    public function ticketReplySend(Request $request, $id)
    {
        ...
        if ($request->replayTicket == 1) {
            ...
            if ($request->hasFile('attachments')) {
                foreach ($request->file('attachments') as $file) {
                    try {
                        ...
                        $notify[] = ['error', 'Non può caricare il tuo ' . $file];
            ...
            $ticket->status = 3;
            $ticket->save();
            $notify[] = ['success', "Il ticket di supporto è stato chiuso con successo!"];
        }
